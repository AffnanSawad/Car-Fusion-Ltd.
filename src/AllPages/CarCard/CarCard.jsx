...
const CarCard = ({car}) => {
  ...
  //    axios.post('http://localhost:5000/myCarts', car_cart_item)
  //    .then( result =>{
  ...
  const handle_Add_To_My_Cart = (car_Info) => {
    ...
    if (user && user.email) {
      ...
      axios.get(`http://localhost:5000/myCarts?email=${user.email}`)
        .then(res => {
          const existingCart = res.data;
          
          const isAlreadyInCart = existingCart.some(item => item.carId === car_cart_item.carId);
